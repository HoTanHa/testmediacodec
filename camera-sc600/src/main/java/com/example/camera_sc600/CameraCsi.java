...
public class CameraCsi {
    ...
    public CameraCsi(int camId, CameraCsiCallback callback) {
        ...
        result = qCarCamera.cameraOpen(inputChannel, inputType);
        if (result != 0) {
            if (LOG_DEBUG) {
                Log.d(TAG, "CameraCsi: Camera open not Success..csi: " + mCamInfo.toString());
            }
            return;
        }

        result = qCarCamera.setVideoColorFormat(inputChannel, QCarCamera.YUV420_NV21);
        if (result != 0) {
            if (LOG_DEBUG) {
